...
> 📅 最後更新：2026-06-02　｜　✅ 所有 API 測試通過　｜　🐳 Docker 三服務一鍵啟動　｜　🔗 前後端完整串接
...
| 航班 API | RapidAPI | 航班編號轉城市／時區資訊 | 免費方案 |
| 匯率 | ExchangeRate-API | 即時匯率查詢與換算 | 免費方案 |
| 容器化 | Docker Compose | 一鍵啟動整個後端環境 | 免費 |

```
travel_wise/
├── docker-compose.yml        ← 一鍵啟動 Redis + Flask 後端 + Vue 前端
├── frontend/                 ← Vue 3 + Vite（npm run dev 或 Docker port 3000）
│   ├── Dockerfile            ← 多階段 build：node:20-alpine → nginx:alpine
│   ├── nginx.conf            ← SPA history mode + gzip + /health
│   └── src/
│       ├── api/              ← axios client + trips / chat API 函式
│       ├── data/travelStore.js ← 全域狀態，API 同步 + localStorage 快取
│       └── components/chat/  ← AI 聊天面板
└── backend/
    ├── Dockerfile            ← python:3.11-slim + gunicorn + curl
    ├── app.py                ← Flask Application Factory
    ├── requirements.txt
    ├── api/routes/           ← 各功能 Blueprint（含新增的 trips.py）
    ├── services/             ← 業務邏輯層
    ├── models/               ← MongoDB Document 結構（含新增的 trip_store.py）
...
```bash
docker compose up --build          # 首次或更新後啟動（重新 build 全部）
docker compose up -d               # 背景啟動（已 build 過）
docker compose down                # 停止並移除容器
docker compose logs -f backend     # 即時查看後端 log
docker compose build frontend && docker compose up -d --no-deps frontend  # 只重建前端
...
services:
  ...
  frontend:
    build: ./frontend            # 多階段 build：Node 20 → Nginx
    container_name: travelwise_frontend
    ports: ["3000:80"]           # Host 3000 → Container 80（Nginx）
    environment:
...
**重點設計：**
- 前端 build 時把 `VITE_API_URL` bake 進 JS bundle，runtime 直接請求後端
- `nginx.conf` 啟用 `try_files` 支援 Vue Router history mode
- 後端安裝 `curl` 供 healthcheck 使用；`--timeout 300` 避免 OpenAI 請求被切斷
- `FLASK_TESTING=true` 開發中跳過 Firebase Token 驗證
...
| `GET /api/flight/info` | ✅ | 需 `RAPIDAPI_KEY` |
...
npm install
npm run dev
# → http://localhost:5173（已自動 proxy /api → localhost:5001）
```
